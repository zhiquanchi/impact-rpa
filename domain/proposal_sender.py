...
class ProposalSender:
    ...
    def send_proposals(
        ...
        Args:
            ...
            send_mode: 发送方式，"card" = 卡片页面直接点按钮；"strip" = 条状列表，
                点击卡片打开侧边栏后再点 Send Proposal 按钮
        ...
        while clicked_count < max_count and total_scrolls < effective_max_scrolls:
            ...
            try:
                # 查找当前可见的目标：卡片模式直接找 Send Proposal 按钮；
                # 条状模式需点击卡片打开侧边栏才能发送
                if send_mode == "card":
                        ...
                        for retry_idx in range(10):
                            if parent:
                                try:
                                    ...
                                    if send_mode == "card":
                                        # 卡片模式：目标就是 Send Proposal 按钮，直接处理弹窗
                                        time.sleep(0.5)
                                    else:
                                        # 条状模式：点击卡片后需等待侧边栏内出现 Send Proposal 按钮
                                        time.sleep(0.5)
                                        send_btn = (
                                    ...
                                    if modal_success:
                                        ...
                                        # 弹窗处理失败，记录警告但不标记目标
                                        logger.warning(
                                            f"弹窗处理失败，跳过此目标 (类别: {selected_tab})"
                                        )
                                        self.console.print(
                                            f"[yellow]⚠ 弹窗处理失败，跳过此目标 (类别: {selected_tab})[/yellow]"
                                        )
                                        # 不增加计数，不标记目标，继续处理下一个

                                    if send_mode != "card":
                                        # 条状模式：无论成功与否，关闭侧边栏准备下一个
                                        self._close_creator_slideout()
                                        time.sleep(0.3)
